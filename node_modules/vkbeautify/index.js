...
function alignComments(code) {
	...
	lines.forEach(line => {
		...
		if (commentPos > -1 && commentPos > maxCommentPos) {
			maxCommentPos = commentPos;
		}
	});
	...
	const alignedLines = lines.map(line => {
		const commentPos = line.indexOf('--');
		if (commentPos > -1) {
			const prefix = line.substring(0, commentPos).trim();
			const comment = line.substring(commentPos);
			const neededSpaces = maxCommentPos - prefix.length;
			return `${prefix}${' '.repeat(neededSpaces)}  ${comment}`;
		}
		return line;
	});
